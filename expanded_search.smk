...
rule all:
	input:
		...
		#        run=config["run"],db_prefix=config["db_prefix"],cluster=config["cluster"]),
		#
		expand("{run}/psiblast_out_cluster{cluster}/merged/merge_db-{db_prefix}_clean_query_hits.fasta",
			run=config["run"],db_prefix=config["db_prefix"],cluster=config["cluster"]),
		#
...
rule sequence_id_cleanup:
	input:
		hits_fasta = "{run}/psiblast_out_cluster{cluster}/merged/merge_db-{db_prefix}_query_hits.fasta"
	output:
		clean_hits_fasta = "{run}/psiblast_out_cluster{cluster}/merged/merge_db-{db_prefix}_clean_query_hits.fasta",
	params:
